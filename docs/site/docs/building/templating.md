...
inside `{{ … }}` is substituted at request time. This page is the whole
engine: the placeholder sources, the expression grammar that pipes them
through transforms, your own custom functions, and how substituted values keep
their types. For the fixture file itself (`status`, `headers`, `body`,
`delay`), see [Fixtures](fixtures.md).
