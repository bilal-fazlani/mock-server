...
| `RESOLVER_HISTORY_TTL_DURATION` | Duration string<br>(default `1d`) | How long a resolver history window survives for a caller that has **no profile** — an ID that resolved from the request under `UNMOCKED_USERS=DEFAULT_MOCK`. Same grammar as `REQUEST_LOG_TTL_DURATION` (`s`, `m`, `h`, `d`). The clock restarts on every call from that ID, and history owned by a real profile or a global-mock selection never expires. See [Code-backed scenario resolvers](../building/dynamic.md#history). |
| `PROFILE_KEY_TTL_DURATION` | Duration string<br>(default `1d`) | How long a captured [profile key mapping](../building/profiles.md#retention-for-callers-with-no-profile) survives when it was captured for a profile ID that has **no profile** — the same unmocked caller as above. Same grammar (`s`, `m`, `h`, `d`). The clock restarts each time that key is captured again; a mapping owned by a real profile never expires, and creating the profile clears the expiry on mappings already captured for it. |
